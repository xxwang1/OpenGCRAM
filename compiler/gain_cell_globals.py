...
def setup_gain_cell():
    """
    Determine the correct custom or parameterized gain_cell for the design.
    """
    # If we have non-1rw ports,
    # and the user didn't over-ride the gain_cell manually,
    # figure out the right gain_cell to use
    if OPTS.gain_cell == "pgain_cell":
        OPTS.gain_cell = "pgain_cell"
        OPTS.dummy_gain_cell = "dummy_pgain_cell"
        OPTS.replica_gain_cell = "replica_pgain_cell"
    else:
        OPTS.num_ports = OPTS.num_rw_ports + OPTS.num_w_ports + OPTS.num_r_ports
        OPTS.gain_cell = "gain_cell_{}port".format(OPTS.num_ports)
    ...
    try:
        ...
        # Use the pgain_cell if we couldn't find a custom gain_cell
        # or its custom replica  gain_cell
        # Use the pgain_cell (and give a warning if not in unit test mode)
        OPTS.gain_cell = "pgain_cell"
        OPTS.dummy_gain_cell = "dummy_pgain_cell"
        OPTS.replica_gain_cell = "replica_pgain_cell"
        if not OPTS.is_unit_test:
            msg = "Didn't find {0}rw {1}w {2}r port gain_cell. ".format(OPTS.num_rw_ports, OPTS.num_w_ports, OPTS.num_r_ports)
            debug.warning("{} Using the parameterized gain_cell which may have suboptimal density.".format(msg))
    debug.info(1, "Using gain_cell: {}".format(OPTS.gain_cell))
...
def report_status():
    ...
    debug.print_raw("Delay Chain Stages: {0}\n".format(OPTS.delay_chain_stages))
    debug.print_raw("Trim Netlist: {OPTS.trim_netlist}\n")
