...
class gain_cell_config:
    """ This is a structure that is used to hold the gain_cell configuration options. """
    ...
    def compute_sizes(self):
        """  Computes the organization of the memory using gain_cell size by trying to make it square."""
        ...
        if not self.words_per_row:
            # Compute the area of the gain_cells and estimate a square bank (excluding auxiliary circuitry)
            self.bank_area = gain_cell.width * gain_cell.height * self.num_bits_per_bank
            self.bank_side_length = sqrt(self.bank_area)

            # Estimate the words per row given the height of the gain_cell and the square side length
            self.tentative_num_cols = int(self.bank_side_length / gain_cell.width)
        ...
        debug.info(1, "Set gain_cell Words Per Row={}".format(OPTS.words_per_row))
