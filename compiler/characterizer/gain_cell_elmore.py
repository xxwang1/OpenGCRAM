...
class gain_cell_elmore(gain_cell_simulation):
    """
    Delay model for the gain_cell which calculates Elmore delays along the gain_cell critical path.
    """
    ...
    def set_params(self):
        ...
        self.params = {}
        # Set the specific functions to use for timing defined in the gain_cell module
        self.params["model_name"] = OPTS.model_name

    def get_lib_values(self, load_slews):
        """
        Return the analytical model results for the gain_cell.
        """
        ...
        if port in self.read_ports:
            bl_name = self.get_rbl_name(self.graph.all_paths, port)
        if port in self.write_ports:
            bl_name = self.get_wbl_name(self.graph.all_paths, port)
        ...
        period_margin = 1.85
        gain_cell_data = {"min_period": (max_delay / 1e3) * 2 * period_margin,
                     "leakage_power": power.leakage}

        debug.info(2, "gain_cell Data:\n{}".format(gain_cell_data))
        debug.info(2, "Port Data:\n{}".format(port_data))

        return (gain_cell_data, port_data)

    def analytical_power(self, load_slews):
        """Get the dynamic and leakage power from the gain_cell"""

        # slews unused, only last load is used
        load = load_slews[-1][0]
        power = self.gain_cell.analytical_power(self.corner, load)
