...
class gain_cell_delay(gain_cell_simulation):
    """
    Functions to measure the delay and power of an gain_cell at a given address and
    data bit.
    ...
    def __init__(self, gain_cell, spfile, corner, output_path=None):
    ...
    def create_read_port_measurement_objects(self):
        ...
        #    if obj.meta_str is gain_cell_op.READ_ZERO:
    ...
    def create_bitline_measurement_objects(self):
        ...
        self.bitline_volt_meas.append(voltage_at_measure("v_rbl_READ_ZERO",
                                                         self.rbl_name))
        self.bitline_volt_meas[-1].meta_str = gain_cell_op.READ_ZERO
        # self.bitline_volt_meas.append(voltage_at_measure("v_wbl_READ_ZERO",
        #                                                  self.wbl_name))
        # self.bitline_volt_meas[-1].meta_str = gain_cell_op.READ_ZERO

        self.bitline_volt_meas.append(voltage_at_measure("v_rbl_READ_ONE",
                                                         self.rbl_name))
        self.bitline_volt_meas[-1].meta_str = gain_cell_op.READ_ONE
        # self.bitline_volt_meas.append(voltage_at_measure("v_wbl_READ_ONE",
        #                                                  self.wbl_name))
        # self.bitline_volt_meas[-1].meta_str = gain_cell_op.READ_ONE
    ...
    def create_write_port_measurement_objects(self):
        ...
        self.write_lib_meas[-1].meta_str = gain_cell_op.WRITE_ZERO
    ...
    def create_debug_measurement_objects(self):
        ...
        self.sen_meas.meta_str = gain_cell_op.READ_ZERO
    ...
    def create_read_bit_measures(self):
        ...
        meas_cycles = (gain_cell_op.READ_ZERO, gain_cell_op.READ_ONE)
    ...
    def create_write_bit_measures(self):
        ...
        meas_cycles = (gain_cell_op.WRITE_ZERO, gain_cell_op.WRITE_ONE)
        for cycle in meas_cycles:
            meas_tag = "a{0}_b{1}_{2}".format(self.probe_address, self.probe_data, cycle.name)
            single_bit_meas = self.get_bit_measures(meas_tag, self.probe_address, self.probe_data)
            for polarity, meas in single_bit_meas.items():
    ...
    def get_bit_measures(self, meas_tag, probe_address, probe_data):
        ...
        if OPTS.top_process == "memchar":
            ...
            (cell_name, cell_inst) = self.gain_cell.get_cell_name(self.gain_cell.name, bit_row, bit_col)
            storage_names = cell_inst.mod.get_storage_net_names()
        debug.check(len(storage_names) == 1, ("Only inverting/non-inverting storage nodes"
...
#        # but is never mentioned otherwise
        ...
        self.bl_path_meas = self.create_delay_path_measures(bitline_path, "rbl")
    ...
    def create_delay_path_measures(self, path, process):
        ...
        for i in range(len(path) - 1):
            ...
            # Some gain_cell logic is hardcoded for only read zeroes, force that here as well.
            path_meas[-1].meta_str = gain_cell_op.READ_ZERO
    ...
    def check_arguments(self):
        ...
        if len(self.probe_address) != self.bank_addr_size:
            debug.error("Probe Address's number of bits does not correspond to given gain_cell", 1)
        ...
        if len(self.read_ports) == 0:
            debug.error("Characterizer does not currently support gain_cells without read ports.", 1)
        if len(self.write_ports) == 0:
            debug.error("Characterizer does not currently support gain_cells without write ports.", 1)
    ...
    def get_delay_measure_variants(self, port, delay_obj):
        ...
        if delay_obj.meta_str == gain_cell_op.READ_ZERO:
        ...
        elif delay_obj.meta_str == gain_cell_op.READ_ONE:
            meas_cycle_delay = self.cycle_times[self.measure_cycles[port][delay_obj.meta_str]]
        else:
    ...
    def get_volt_when_measure_variants(self, port, volt_meas):
        ...
        t_trig = self.cycle_times[self.measure_cycles[port][gain_cell_op.READ_ZERO]]
    ...
    def load_pex_net(self, net: str):
        ...
        prefix = (self.gain_cell_instance_name + OPTS.hier_seperator).lower()
    ...
    def write_simulation_saves(self):
        ...
        probe_nets = set()
        gain_cell_name = self.gain_cell_instance_name
        ...
        for port in set(self.targ_read_ports + self.targ_write_ports):
            ...
            probe_nets.add("{0}{2}w_en{1}".format(self.gain_cell_instance_name, port, OPTS.hier_seperator))
            probe_nets.add("{0}{3}Xbank0{3}Xport_data{1}{3}Xwrite_driver_array{1}{3}Xwrite_driver{2}{3}en_bar".format(self.gain_cell_instance_name,
                                                                                                                      port,
                                                                                                                      self.bitline_column,
                                                                                                                      OPTS.hier_seperator))
            # probe_nets.add("{0}{3}Xbank0{3}br_{1}_{2}".format(self.gain_cell_instance_name,
                                                            #   port,
                                                            #   self.bitline_column,
                                                            #   OPTS.hier_seperator))
            if not OPTS.use_pex:
                continue
            probe_nets.add(
                "{0}{3}vdd_Xbank0_Xgain_cell_array_xgain_cell_array_xbit_r{1}_c{2}".format(gain_cell_name,
                                                                                       row,
                                                                                       col - 1,
                                                                                       OPTS.hier_seperator))
            probe_nets.add(
    ...
    def check_read_debug_measures(self, port):
        ...
        bl_vals = {}
        # br_vals = {}
        if port in self.read_ports: self.bl_name = self.rbl_name
        elif port in self.write_ports: self.bl_name = self.wbl_name
    ...
    def check_bit_measures(self, bit_measures, port):
        ...
        for polarity, meas_list in bit_measures.items():
            for meas in meas_list:
                ...
                if (meas_cycle == gain_cell_op.READ_ZERO and polarity == bit_polarity.NONINVERTING) or\
                   (meas_cycle == gain_cell_op.READ_ONE and polarity == bit_polarity.INVERTING):
                    success = val < self.vdd_voltage / 2
                elif (meas_cycle == gain_cell_op.READ_ZERO and polarity == bit_polarity.INVERTING) or\
                     (meas_cycle == gain_cell_op.READ_ONE and polarity == bit_polarity.NONINVERTING):
                    success = val > self.vdd_voltage / 2
                elif (meas_cycle == gain_cell_op.WRITE_ZERO and polarity == bit_polarity.INVERTING) or\
                     (meas_cycle == gain_cell_op.WRITE_ONE and polarity == bit_polarity.NONINVERTING):
                    success = val > self.vdd_voltage / 2
                elif (meas_cycle == gain_cell_op.WRITE_ONE and polarity == bit_polarity.INVERTING) or\
                     (meas_cycle == gain_cell_op.WRITE_ZERO and polarity == bit_polarity.NONINVERTING):
                    success = val < self.vdd_voltage / 2
                if not success:
    ...
    def run_power_simulation(self):
        """
        This simulates a disabled gain_cell to get the leakage power when it is off.
        """
    ...
    def try_period(self, feasible_delays):
        ...
        for port in self.targ_read_ports:
            ...
            for dname in self.delay_meas_names:
                ...
                if self.gain_cell.col_addr_size>0 and "slew" in dname:
                    continue
    ...
    def prepare_netlist(self):
        ...
        if OPTS.trim_netlist:
            ...
            if OPTS.top_process != "memchar":
                self.gain_cell.sp_write(self.trim_sp_file, lvs=False, trim=True)
        else:
            # The non-reduced netlist file when it is disabled
            self.trim_sp_file = "{0}gain_cell.sp".format(self.output_path)

        # The non-reduced netlist file for power simulation
        self.sim_sp_file = "{0}gain_cell.sp".format(self.output_path)
...
#                if read:
#                    if cycle == "one":
#                        meas.meta_str = gain_cell_op.READ_ONE
#                    else:
#                        meas.meta_str = gain_cell_op.READ_ZERO
#                    self.read_bit_meas[polarity].append(meas)
#                    self.read_meas_lists[-1].append(meas)
#                else:
#                    if cycle == "one":
#                        meas.meta_str = gain_cell_op.WRITE_ONE
#                    else:
#                        meas.meta_str = gain_cell_op.WRITE_ZERO
#                    self.write_bit_meas[polarity].append(meas)
#                    self.write_meas_lists[-1].append(meas)
    ...
    def set_spice_names(self):
        ...
        self.rbl_name = OPTS.rbl_format.format(name=self.gain_cell.name,
                                             hier_sep=OPTS.hier_seperator,
                                             row="{}",
                                             col=self.bitline_column)
        self.wbl_name = OPTS.wbl_format.format(name=self.gain_cell.name,
                                             hier_sep=OPTS.hier_seperator,
                                             row="{}",
                                             col=self.bitline_column)
        self.sen_name = OPTS.sen_format.format(name=self.gain_cell.name,
                                               hier_sep=OPTS.hier_seperator)
        self.cell_name = OPTS.cell_format.format(name=self.gain_cell.name,
                                                 hier_sep=OPTS.hier_seperator,
                                                 row="{}",
                                                 col="{}")
    ...
    def analyze(self, probe_address, probe_data, load_slews):
        """
        Main function to characterize an gain_cell for a table. Computes both delay and power characterization.
        """
        ...
        char_gain_cell_data["min_period"] = round_time(min_period)
        ...
        char_gain_cell_data["leakage_power"]=full_array_leakage
        ...
        # char_gain_cell_data["bl_path_measures"] = bl_delays
        # char_gain_cell_data["sen_path_measures"] = sen_delays
        # char_gain_cell_data["bl_path_names"] = bl_names
        # char_gain_cell_data["sen_path_names"] = sen_names
        ...
        return (char_gain_cell_data, char_port_data)
    ...
    def calculate_inverse_address(self):
        ...
        inverse_address = ""
        for c in self.probe_address[self.gain_cell.col_addr_size:]: # invert everything else
            if c=="0":
                inverse_address += "1"
            elif c=="1":
                inverse_address += "0"
    ...
    def gen_test_cycles_one_port(self, read_port, write_port):
        ...
        self.add_write("W data 0 address {0} to write value".format(self.probe_address),
                       ...
                       wmask_ones,
                       write_port)
        self.measure_cycles[write_port][gain_cell_op.WRITE_ZERO] = len(self.cycle_times) - 1
        ...
        self.add_read("R data 0 address {0} to check W0 worked".format(self.probe_address),
                      self.probe_address,
                      read_port)
        self.measure_cycles[read_port][gain_cell_op.READ_ZERO] = len(self.cycle_times) - 1
        ...
        self.add_write("W data 1 address {0} to write value".format(self.probe_address),
                       ...
                       wmask_ones,
                       write_port)
        self.measure_cycles[write_port][gain_cell_op.WRITE_ONE] = len(self.cycle_times) - 1
        ...
        self.add_read("R data 1 address {0} to check W1 worked".format(self.probe_address),
                      self.probe_address,
                      read_port)
        self.measure_cycles[read_port][gain_cell_op.READ_ONE] = len(self.cycle_times) - 1
    ...
    def set_stimulus_variables(self):
        gain_cell_simulation.set_stimulus_variables(self)
