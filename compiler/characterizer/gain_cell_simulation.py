...
class gain_cell_simulation():

    def __init__(self, gain_cell, spfile, corner):
        self.gain_cell = gain_cell

        self.name = self.gain_cell.name
        self.word_size = self.gain_cell.word_size
        self.bank_addr_size = self.gain_cell.bank_addr_size
        self.write_size = self.gain_cell.write_size
        self.num_spare_rows = self.gain_cell.num_spare_rows
        if not self.gain_cell.num_spare_cols:
            self.num_spare_cols = 0
        else:
            self.num_spare_cols = self.gain_cell.num_spare_cols
        if not spfile:
            self.sp_file = OPTS.openram_temp + "gain_cell.sp"
        else:
            self.sp_file = spfile

        self.all_ports = self.gain_cell.all_ports
        self.readwrite_ports = self.gain_cell.readwrite_ports
        self.read_ports = self.gain_cell.read_ports
        self.write_ports = self.gain_cell.write_ports
        self.words_per_row = self.gain_cell.words_per_row
        self.num_rows = self.gain_cell.num_rows
        self.num_cols = self.gain_cell.num_cols
    ...
    def create_signal_names(self):
        ...
        debug.check(len(self.gain_cell.pins) == len(self.pins),
                    "Number of pins generated for characterization \
                    do not match pins of gain_cell\ngain_cell.pins = {0}\npin_names = {1}".format(self.gain_cell.pins,
                                                                                        self.pins))
    ...
    def get_data_bit_column_number(self, probe_address, probe_data):
        ...
        if self.gain_cell.col_addr_size>0:
            col_address = int(probe_address[0:self.gain_cell.col_addr_size], 2)
        else:
            col_address = 0
        bl_column = int(self.gain_cell.words_per_row * probe_data + col_address)
        return bl_column
    ...
    def get_address_row_number(self, probe_address):
        ...
        return int(probe_address[self.gain_cell.col_addr_size:], 2)
    ...
    def gen_pin_names(self, port_signal_names, port_info, abits, dbits):
        """Creates the pins names of the gain_cell based on the no. of ports."""
        # This may seem redundant as the pin names are already defined in the gain_cell. However, it is difficult
    ...
    def get_column_addr(self):
        ...
        return self.probe_address[:self.gain_cell.col_addr_size]

    def add_graph_exclusions(self):
        """
        Exclude portions of gain_cell from timing graph which are not relevant
        """
    ...
    def set_internal_spice_names(self):
        ...
        if not OPTS.use_pex or (OPTS.use_pex and OPTS.pex_exe[0] == "calibre"):
            ...
            if port in self.read_ports:
                rbl_name_port = self.get_rbl_name(self.graph.all_paths, port)
                # port_pos = -1 - len(str(column_addr)) - len(str(port))

                if rbl_name_port.endswith(str(port) + "_" + str(self.bitline_column)): # single port gain_cell case, bl will not be numbered eg bl_0
                    self.rbl_name = rbl_name_port
                else:
                    self.rbl_name = rbl_name_port
                    debug.warning("Error occurred while determining bitline names. Can cause faults in simulation.")
            if port in self.write_ports:
                wbl_name_port = self.get_wbl_name(self.graph.all_paths, port)
                # port_pos = -1 - len(str(column_addr)) - len(str(port))

                if wbl_name_port.endswith(str(port) + "_" + str(self.bitline_column)): # single port gain_cell case, bl will not be numbered eg bl_0
                    self.wbl_name = wbl_name_port
                else:
                    self.wbl_name = wbl_name_port
                    debug.warning("Error occurred while determining bitline names. Can cause faults in simulation.")

            # if br_name_port.endswith(str(port) + "_" + str(self.bitline_column)): # single port gain_cell case, bl will not be numbered eg bl_0
            ...
            if port in self.read_ports:
                self.rbl_name = "rbl{0}_{1}".format(port, OPTS.word_size - 1)
            if port in self.write_ports:
                self.wbl_name = "wbl{0}_{1}".format(port, OPTS.word_size - 1)
    ...
    def get_sen_name(self, paths, assumed_port=None):
        ...
        sa_mods = factory.get_mods(OPTS.gain_cell_sense_amp)
    ...
    def create_graph(self):
        """
        Creates timing graph to generate the timing paths for the gain_cell output.
        """

        # Make exclusions dependent on the bit being tested.
        self.gain_cell.clear_exclude_bits() # Removes previous bit exclusions
        self.gain_cell.graph_exclude_bits(self.wordline_row, self.bitline_column)
        port=self.read_ports[0] # FIXME, port_data requires a port specification, assuming single port for now
        if self.words_per_row > 1:
            self.gain_cell.graph_clear_column_mux(port)
            self.gain_cell.graph_exclude_column_mux(self.bitline_column, port)

        # Generate new graph every analysis as edges might change depending on test bit
        self.graph = timing_graph()
        self.gain_cell_instance_name = "X{}".format(self.gain_cell.name)
        self.gain_cell.build_graph(self.graph, self.gain_cell_instance_name, self.pins)

    def get_rbl_name_search_exclusions(self):
        """
    ...
    def get_wbl_name_search_exclusions(self):
        """
        Gets the mods as a set which should be excluded while searching for name.
        """
    ...
    def get_alias_in_path(self, paths, internal_net, mod, exclusion_set=None):
        ...
        net_found = False
        for path in paths:
            aliases = self.gain_cell.find_aliases(self.gain_cell_instance_name, self.pins, path, internal_net, mod, exclusion_set)
    ...
    def get_rbl_name(self, paths, port):
        """
        ...
        if OPTS.tech_name == "sky130" and len(self.all_ports) == 1:
            cell_mod = factory.create(module_type=OPTS.gain_cell, version="opt1")
        else:
            cell_mod = factory.create(module_type=OPTS.gain_cell)
        if port in self.read_ports:
            cell_rbl = cell_mod.get_rbl_name(port)
        if port in self.write_ports:
            cell_wbl = cell_mod.get_wbl_name(port)
        # cell_br = cell_mod.get_br_name(port)

        if port in self.read_ports:
            rbl_names = []
            exclude_set = self.get_rbl_name_search_exclusions()
    ...
    def get_wbl_name(self, paths, port):
        if OPTS.tech_name == "sky130" and len(self.all_ports) == 1:
            cell_mod = factory.create(module_type=OPTS.gain_cell, version="opt1")
        else:
            cell_mod = factory.create(module_type=OPTS.gain_cell)
        ...
        if port in self.write_ports:
            wbl_names = []
            exclude_set = self.get_wbl_name_search_exclusions()
