...
class router(router_tech):
    ...
    def add_path(self, path, multiple_via5=False):
        """ Add the route path to the layout. """

        nodes = self.prepare_path(path)
        shapes = self.add_route(nodes, multiple_via5)
        return shapes
    ...
    def add_route(self, nodes, multiple_via5=False):
        """
        ...
        via_count = 0
        total_via_count = 0
        multiple_via5_added = 0
        for i in range(0, len(nodes) - 1):
            direction = nodes[i].get_direction(nodes[i + 1])
            if direction == (1, 1): total_via_count +=1
        for i in range(0, len(nodes) - 1):
            
            start = nodes[i].center
            end = nodes[i + 1].center
            direction = nodes[i].get_direction(nodes[i + 1])
            print("The i th node, start, end and direction = ", nodes[i], start, end, direction)
            diff = start - end
            ...
            if direction == (1, 1): # Via
                ...
                if (multiple_via5 == "SH" or multiple_via5 == "SV"):
                    if via_count == 0 and multiple_via5_added == 0: 
                        via = self.design.add_via_center(layers=self.layers,
                                                    offset=offset,
                                                    multiple_via5=multiple_via5[1])
                        via_count += 1
                        multiple_via5_added += 1
                    else:
                        via = self.design.add_via_center(layers=self.layers,
                                                    offset=offset,
                                                    multiple_via5=False)
                        via_count += 1
                elif (multiple_via5 == "TH" or multiple_via5 == "TV"):
                    if (via_count == total_via_count - 1) and multiple_via5_added == 0: 
                        via = self.design.add_via_center(layers=self.layers,
                                                    offset=offset,
                                                    multiple_via5=multiple_via5[1])
                        via_count += 1
