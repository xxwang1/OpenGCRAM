...
class instance(geometry):
    ...
    def reverse_transformation_bitcell(self, cell_name):
        ...
        rbl_offsets = [] # bl to cell offset
        wbl_offsets = [] # br to cell offset
        rbl_meta = [] # bl offset metadata (row,col,name)
        wbl_meta  = [] # br offset metadata (row,col,name)
        ...
        def walk_subtree(node):
            ...
            if node.mod.name == cell_name:
                ...
                SN_x = normalized_storage_nets[0][0]
                SN_y = normalized_storage_nets[0][1]
                ...
                if node.mirror == 'MX':
                    ...
                    for pair in range(len(normalized_rbl_offsets)):
                        normalized_rbl_offsets[pair] = (normalized_rbl_offsets[pair][0],
                                                       -1 * normalized_rbl_offsets[pair][1])

                    for pair in range(len(normalized_wbl_offsets)):
                        normalized_wbl_offsets[pair] = (normalized_wbl_offsets[pair][0],
                                                       -1 * normalized_wbl_offsets[pair][1])

                SN_offsets.append([SN_x, SN_y])
                # Q_bar_offsets.append([Q_bar_x, Q_bar_y])

                rbl_offsets.append(normalized_rbl_offsets)
                wbl_offsets.append(normalized_wbl_offsets)

                rbl_meta.append(cell_rbl_meta)
                wbl_meta.append(cell_wbl_meta)
        ...
        return(origin_offsets, SN_offsets, rbl_offsets, wbl_offsets, rbl_meta, wbl_meta)
...
class label(geometry):
    ...
    def __init__(self, text, lpp, offset, zoom=None, snap_to_grid=True):
        """Initializes a text label for specified layer"""
        super().__init__(lpp)
        self.name = "label"
        self.text = text
        ini_offset_x = offset[0]
        self.offset = vector(offset).snap_to_grid()
        if not snap_to_grid: self.offset = vector(ini_offset_x, self.offset.y)
...
class rectangle(geometry):
    ...
    def __init__(self, lpp, offset, width, height, snap_to_grid=True):
