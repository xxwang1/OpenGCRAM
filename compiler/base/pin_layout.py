...
class pin_layout:
    ...
    def __init__(self, name, rect, layer_name_pp, snap_to_grid=True):
        # print("********************************************************************")
        # print("pin mname =", name)
        # print("pin rect = ", rect)
        # print("layer name pp =", layer_name_pp)
        # print("********************************************************************")
        self.name = name
        ...
        ini_0_x = vector(rect[0]).x
        ini_1_x = vector(rect[1]).x
        self._rect = [x.snap_to_grid() for x in self.rect]
        if not snap_to_grid: self._rect = [vector(ini_0_x, self._rect[0].y), vector(ini_1_x, self._rect[1].y)]
        ...
        # else it is required to be a lpp
            ...
            for (layer_name, lpp) in valid_layers.items():
                ...
                try:
                    ...
                    # print("****************************************")
                    # print("layer override =", layer_override)
                    # print("layer override name =", layer_override_name)
                    # print("layer override[name] = ", layer_override[name])
                    # print("****************************************")
    ...
    def inflate(self, spacing=None, multiple=0.5):
        ...
        if not spacing:
            if self.layer[-1] == "p":
                ...
                layer = self.layer
            spacing = multiple*drc("{0}_to_{0}".format(layer))
    ...
    def center(self):
        ...
        else: return vector(0.5*(self.rect[0].x+self.rect[1].x),
                      0.5*(self.rect[0].y+self.rect[1].y)).snap_to_grid()
    ...
    def same_lpp(self, lpp1, lpp2):
        ...
        if lpp1[1] == None or lpp2[1] == None:
            if lpp1[0] == 131 and lpp2[0] ==31:
                return True
            elif lpp1[0] == 132 and lpp2[0] == 32:
                return True
            elif lpp1[0] == 133 and lpp2[0] == 33:
                return True
            elif lpp1[0] == 134 and lpp2[0] == 34:
                return True
            elif lpp1[0] == 135 and lpp2[0] == 35:
                return True
            elif lpp1[0] == 136 and lpp2[0] == 36:
                return True
            else:
