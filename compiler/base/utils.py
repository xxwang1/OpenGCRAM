...
def sqrt(number):
    return round_to_grid(math.sqrt(number))
...
def round_to_grid(number):
    ...
    number_grid = (round(round((number / grid), 2), 0))
    number_off = number_grid * grid
    # print("round_to_grid grid, number, number_off = ", grid, number, number_off)
    return number_off
    # return number
def round_to_grid_area(number):
    """
    Rounds an arbitrary number to the grid.
    """
    grid = tech.drc["grid"]
    # this gets the nearest integer value
    number_grid = (round(round((number / grid), 2), 1))
    number_off = number_grid * grid
    # print("round_to_grid grid, number, number_off = ", grid, number, number_off)
    return number_off
