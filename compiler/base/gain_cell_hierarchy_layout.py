...
from .utils import round_to_grid, ceil, sqrt, round_to_grid_area
from . import gain_cell_geometry as geometry
...
class layout():
    ...
    def setup_drc_constants(layout):
        ...
        print("Running setup drc constants")
        print("drc.keys= ", drc.keys())
        for rule in drc.keys():
            # Single layer width rules
            print("rule, number= ", rule, drc(rule))
        ...
        debug.info(level, "m5_width".format(layout.m5_width))
        debug.info(level, "m5_space".format(layout.m5_space))
        debug.info(level, "m6_width".format(layout.m6_width))
        debug.info(level, "m6_space".format(layout.m6_space))
        ...
        debug.info(level, "contact_width".format(layout.contact_width))
        debug.info(level, "poly_enclose_contact".format(layout.poly_enclose_contact))
        ...
        debug.info(level, "implant_enclose_poly".format(layout.implant_enclose_poly))
        debug.info(level, "active_to_poly".format(layout.active_to_poly))
        debug.info(level, "nwell_space".format(layout.nwell_space))
        debug.info(level, "m4_extend_via4".format(layout.m4_extend_via4))
        debug.info(level, "via5_space".format(layout.via5_space))
        debug.info(level, "via5_width".format(layout.via5_width))
        # debug.info(level, "implant_space".format(layout.implant_space))
        # debug.info(level, "implant_space".format(layout.implant_space))
        debug.info(level, "via5_pitch".format(layout.via5_width + layout.via5_space))
        debug.info(level, "m6_enclose_via5".format(layout.m6_enclose_via5))
    @classmethod
    ...
    def compute_pitch(layer, preferred=True):
        ...
        for stack in tech_layer_stacks:
            ...
            if stack[2] == layer:
                if layer == "m6":
                    print("Running compute_layer_pitch for ", stack[::-1])
    ...
    def compute_layer_pitch(layer_stack, preferred):
        ...
        try:
            if layer1 == "poly" or layer1 == "active":
                ...
                if layer_stack == ('m6', 'via5', 'm5'):
                    print("contact1 should be m6_via")
                    print("contact1 = ", getattr(layout, layer1 + "_via"))
        ...
        if preferred:
            if preferred_directions[layer1] == "V":
                ...
                if layer_stack == ('m6', 'via5', 'm5'):
                    print("contact1 width = ", contact_width)
            else:
            ...
            if preferred_directions[layer1] == "V":
                ...
                contact_width = contact1.first_layer_width
        contact_width = max(contact_width, getattr(layout, layer1 + "_width"))
        layer_space = getattr(layout, layer1 + "_space")

        pitch = contact_width + layer_space
        if layer_stack == ('m6', 'via5', 'm5'):
            print("layer_space = ", layer_space)
            print("m6 pitch = ", pitch)
    ...
    def find_highest_layer_coords(self, layer):
        ...
        print("find_highest_layer_coords = ", layerNumber, layer)
        try:
            # print("try find_highest_layer_coords = ", layerNumber, layer)
            # highest = []
            # for obj in self.objs:
            #     if obj.layerNumber == layerNumber
            #         # highestx = max(obj.rx(), highestx)
            #         highest.append(obj.rx())
            # highestx = max(highest)
            highestx = max(obj.rx() for obj in self.objs if (obj.layerNumber == layerNumber and obj.name != "label"))
        except ValueError:
            highestx =0
        try:
            highesty = max(obj.uy() for obj in self.objs if (obj.layerNumber == layerNumber and obj.name != "label"))
        except ValueError:
            highesty = 0
    ...
    def find_lowest_layer_coords(self, layer):
        ...
        layerNumber = tech_layer[layer][0]
        try:
            lowestx = min(obj.lx() for obj in self.objs if (obj.layerNumber == layerNumber and obj.name != "label"))
        except ValueError:
            lowestx = 0
        try:
            lowesty = min(obj.by() for obj in self.objs if (obj.layerNumber == layerNumber and obj.name != "label"))
        except ValueError:
            lowesty = 0
    ...
    def add_rect(self, layer, offset, width=None, height=None, snap_to_grid=True):
        """
        Adds a rectangle on a given layer,offset with width and height
        """
        print("Running add_rect")
        print("add_rect layer = ", layer)
        if not width:
        ...
        print("add_rect layer width, height= ", width, height)
    ...
    def add_rect_center(self, layer, offset, width=None, height=None):
        ...
        print("add rect_center corrected_offset, width, height = ", corrected_offset, width, height)
        self.objs.append(geometry.rectangle(lpp,
                                            corrected_offset,
                                            width,
                                            height))
    ...
    def add_segment_center(self, layer, start, end, width=None):
        ...
        if start.x != end.x and start.y != end.y:
            ...
            if (end.x - start.x)>0: width_ = round(end.x - start.x, 3)
            else: width_ = round(- end.x + start.x, 3) * (-1)
            ...
            if (end.y - start.y) > 0: height = round(end.y - start.y, 3)
            else: height = round(- end.y + start.y, 3) * (-1)
            return self.add_rect(layer,
                                 start - offset,
                                 width,
                                 height)
    ...
    def get_pin(self, text):
        ...
        print("************************************************")
        print("self.pin_map = ", self.pin_map)
        print("self.pin_map name = ", self.pin_map[name])
        # print("self.pin_map layer = ", self.pin_map[name].layer)
        print("************************************************")
        try:
    ...
    def get_pins(self, text):
        ...
        if name in self.pin_map.keys():
            for pin in self.pin_map[name]:
                print("self.pin = ", pin)
                print("self.pin_map layer = ", pin.layer)
...
#                                          width=via_width)
    ...
    def route_horizontal_pins(self, name, insts=None, layer=None, xside="cx", yside="cy", full_width=True, new_name=None, multiple_via4=True, snap_to_grid=True):
        ...
        print("route horizontal pins bins = ", bins)
        for y, v in bins.items():
            if len(v) < 2:
                debug.warning("Pins don't align well so copying pins instead of connecting with pin.")
                for inst,pin in v:
    ...
    def add_layout_pin_segment_center(self, text, layer, start, end, width=None, snap_to_grid=True):
        ...
        if max(bbox_width, layer_width) > 0: width = round(max(bbox_width, layer_width), 3)
    ...
    def add_label(self, text, layer, offset=[0, 0], zoom=None, snap_to_grid=True):
        """Adds a text label on the given layer,offset, and zoom level"""
        debug.info(5, "add label " + str(text) + " " + layer + " " + str(offset))
        lpp = tech_layer[layer]
        self.objs.append(geometry.label(text, lpp, offset, zoom, snap_to_grid))
        return self.objs[-1]

    def add_path(self, layer, coordinates, width=None, minarea=None):
        ...
        if minarea:
            ...
            if preferred_directions[layer] == "V":
                ini_width = abs(coordinates[0].x - coordinates[1].x)
                ini_height = abs(coordinates[0].y - coordinates[1].y)
                if width: 
                    ini_width = max(ini_width, width, min_width)
                else: 
                    ini_width = max(ini_width, min_width)
                
                new_height = ceil(max(min_area / ini_width, ini_height))
                new_width = ini_width
                if new_height > ini_height:
                    if coordinates[0].y <= coordinates[1].y:
                        new_coordinates = [vector(coordinates[0].x, coordinates[0].y - 0.5 * (new_height - ini_height)),
                                        vector(coordinates[1].x, coordinates[1].y + 0.5 * (new_height - ini_height))]
                    else:
                        new_coordinates = [vector(coordinates[0].x, coordinates[0].y + 0.5 * (new_height - ini_height)),
                                        vector(coordinates[1].x, coordinates[1].y - 0.5 * (new_height - ini_height))]
                else:
                ...
                ini_height = abs(coordinates[0].y - coordinates[1].y)
                if width: 
                    ini_height = max(ini_height, width, min_width)
                else: 
                    ini_height = max(ini_height, min_width)
                new_width = ceil(max(min_area / ini_height, ini_width))
                new_height = ini_height
                if new_width > ini_width:
                    if coordinates[0].x <= coordinates[1].x:
                        new_coordinates = [vector(coordinates[0].x - 0.5 * (new_width - ini_width), coordinates[0].y),
                                        vector(coordinates[1].x + 0.5 * (new_width - ini_width), coordinates[1].y)]
                    else:
                        new_coordinates = [vector(coordinates[0].x + 0.5 * (new_width - ini_width), coordinates[0].y),
                                        vector(coordinates[1].x - 0.5 * (new_width - ini_width), coordinates[1].y)]
                else:
                    new_coordinates = coordinates
            print("add_path min_area, new_height, new_width = ", min_area, new_height, new_width)
            debug.check(min_area <= (new_height*new_width), "Min area violated.")
        else:
            new_width = width
            new_coordinates = coordinates
    ...
    def add_via_center_single(self, layers, offset, directions=None, size=[1, 1], implant_type=None, well_type=None):
        """
    ...
    def add_via_center(self, layers, offset, directions=None, size=[1, 1], implant_type=None, well_type=None, multiple_via4=True, multiple_via5=None):
        ...
        accounting for mirroring and rotation.
        """
        if not (("m4" in layers and "m5" in layers) or ("m5" in layers and "m6" in layers)) or ((not multiple_via4) and (not multiple_via5)):
            via = factory.create(module_type="contact",
                                layer_stack=layers,
                                dimensions=size,
                                directions=directions,
            ...
            height = via.height
            width = via.width
            if (directions==("V", "V") or directions==None) and (implant_type=="p" and well_type=="p") or (implant_type=="n" and well_type=="n"):
                height = via.second_layer_height
            else:
                height = via.height
            #     initial_offset_y = max(0.5 * self.implant_width + self.implant_enclose_active,
            #                       self.get_tx_insts("nmos")[0].by())
            #     contact_yoffset = max(initial_offset_y, self.implant_enclose_active + self.implant_space * 0.5 + via.height - via.second_layer_height)
            #     contact_yoffset += 0.5 * (via.height - via.second_layer_height) + offset.y - initial_offset_y
            print("via height, width, initial offset = ", height, width, offset)
            corrected_offset = offset + vector(-0.5 * width,
                                            -0.5 * height)
            print("layer_stack, corrected_offset, via = ", layers, corrected_offset, via)
            inst = self.add_inst(name=via.name,
                                mod=via,
                                offset=corrected_offset)
            # We don't model the logical connectivity of wires/paths
            ...
            # if (cur_layer == "m4" and (to_layer == "m6" or to_layer == "m5")) or (cur_layer == "m5" and to_layer == "m4"):
            #     print("cur_layer=m4, curr_stack=", curr_stack)
                # offset_left = [0,0]
                # print("add_via_stack_center size[0]= ", size[0])
                # offset_left[0] = offset[0] - self.m4_space/2 - 0.13
                # offset_left[1] = offset[1]
                # offset_right = [0,0]
                # offset_right[0] = offset[0] + self.m4_space/2
                # offset_right[1] = offset[1]
            if "m4" in layers and "m5" in layers:
                if multiple_via4:
                    offset_left1 = vector(offset[0] - 0.5 * self.m4_pitch, offset[1]).snap_to_grid()
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_left1,
                                            directions=directions,
                                            implant_type=implant_type,
                                            well_type=well_type)
                    print("via = ", via)
                    print("via.mod.first_layer_width, height = ", via.mod.first_layer_width, via.mod.first_layer_height)
                    # if min_area or cur_layer in intermediate_layers:
                    # self.add_min_area_rect_center(layers[0],
                    #                             offset,
                    #                             via.mod.first_layer_width,
                    #                             via.mod.first_layer_height)
                    # offset_tl = [0,0]
                    # offset_tl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1]).snap_to_grid()
                    offset_right1 = vector(offset[0] + 0.5 * self.m4_pitch, offset[1]).snap_to_grid()
                    # offset_tl[1] = offset[1]
                    # offset_tl = offset_tl.snap_to_grid()
                    print("offset, offset_topleft = ", offset, offset_right1)
                
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_right1,
                                            directions=directions,
                                            implant_type=implant_type,
                                            well_type=well_type)
                    # if min_area or cur_layer in intermediate_layers:
                    ...
                    #                                 via.mod.first_layer_height)
                    # offset_bl = [0,0]
                    # offset_bl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1] - self.m4_space - self.m4_width).snap_to_grid()
                    offset_right2 = vector(offset[0] + 1.5 * self.m4_pitch, offset[1]).snap_to_grid()
                    # offset_bl[1] = offset[1] - self.m4_space - self.m4_width
                    # offset_bl = offset_bl.snap_to_grid()
                    # print("offset, offset_left = ", offset, offset_left)
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_right2,
                                            directions=directions,
                                            implant_type=implant_type,
                                            well_type=well_type)
                    # if min_area or cur_layer in intermediate_layers:
                    ...
                    #                                 via.mod.first_layer_height)
                    # offset_br = [0,0]
                    offset_left2 = vector(offset[0] - 1.5 * self.m4_pitch, offset[1]).snap_to_grid()
                    # offset_br[1] = offset[1] - self.m4_space - self.m4_width
                    # offset_br = offset_br.snap_to_grid()
                    # print("offset, offset_left = ", offset, offset_left)
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_left2,
                    ...
                    #                                 via.mod.first_layer_width,
                    #                                 via.mod.first_layer_height)
                    # if "m4" in layers and "m5"
                    self.add_rect_center(layer=layers[0],
                                        offset=offset,
                                        height=via.mod.first_layer_height,
                                        width = 4 * self.m4_pitch - self.m4_width + 2 * self.m4_extend_via4)
                    self.add_rect_center(layer=layers[-1],
                                        offset=offset,
                                        height=via.mod.second_layer_height,
                                        width = 4 * self.m4_pitch - self.m4_width + 2 * self.m5_extend_via4)
                    return via
                else:
                    via = factory.create(module_type="contact",
                                layer_stack=layers,
                    ...
                    self.connect_inst([])
                    return inst
            elif "m5" in layers and "m6" in layers:
                via = factory.create(module_type="contact",
                                layer_stack=layers,
                                dimensions=size,
                                directions=directions,
                ...
                self.connect_inst([])
                if multiple_via5 == "V":
                    offset_btm1 = vector(offset[0], offset[1] - (self.via5_width + self.m6_enclose_via5 * 2 + self.m6_space)).snap_to_grid()
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_btm1,
                                            directions=directions,
                    ...
                    # offset_tl = [0,0]
                    # offset_tl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1]).snap_to_grid()
                    offset_top1 = vector(offset[0], offset[1] + (self.via5_width + self.m6_enclose_via5 * 2 + self.m6_space)).snap_to_grid()
                    # offset_tl[1] = offset[1]
                    # offset_tl = offset_tl.snap_to_grid()
                    # print("offset, offset_topleft = ", offset, offset_top1)
                    ...
                    # offset_bl = [0,0]
                    # offset_bl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1] - self.m4_space - self.m4_width).snap_to_grid()
                    # offset_top2 = vector(offset[0], offset[1] + 1.5 * self.m4_pitch).snap_to_grid()
                    # # offset_bl[1] = offset[1] - self.m4_space - self.m4_width
                    # # offset_bl = offset_bl.snap_to_grid()
                    # # print("offset, offset_left = ", offset, offset_left)
                    # via = self.add_via_center_single(layers=layers,
                    ...
                    # #                                 via.mod.first_layer_height)
                    # # offset_br = [0,0]
                    # offset_br = vector(offset[0], offset[1] - 1.5 * self.m4_pitch).snap_to_grid()
                    # # offset_br[1] = offset[1] - self.m4_space - self.m4_width
                    # # offset_br = offset_br.snap_to_grid()
                    # # print("offset, offset_left = ", offset, offset_left)
                    # via = self.add_via_center_single(layers=layers,
                    ...
                    self.add_rect_center(layer=layers[0],
                                        offset=offset,
                                        width=via.mod.first_layer_width,
                                        height = 2 * self.m6_pitch - self.m6_width)
                    self.add_rect_center(layer=layers[-1],
                                        offset=offset,
                                        width=via.mod.second_layer_width,
                                        height = 2 * self.m6_pitch - self.m6_width)
                    # corrected_offset = offset + vector(-0.5 * via.width,
                    #                                 -0.5 * via.height)
                    # inst = self.add_inst(name=via.name,
                    #                     mod=via,
                    #                     offset=corrected_offset)
                    ...
                    offset_left1 = vector(offset[0] - (self.via5_width + self.m6_enclose_via5 * 2 + self.m6_space), offset[1]).snap_to_grid()
                    via = self.add_via_center_single(layers=layers,
                                            size=size,
                                            offset=offset_left1,
                                            directions=directions,
                    ...
                    # offset_tl = [0,0]
                    # offset_tl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1]).snap_to_grid()
                    offset_right1 = vector(offset[0] + (self.via5_width + self.m6_enclose_via5 * 2 + self.m6_space), offset[1]).snap_to_grid()
                    # offset_tl[1] = offset[1]
                    # offset_tl = offset_tl.snap_to_grid()
                    # print("offset, offset_topleft = ", offset, offset_top1)
                    ...
                    self.add_rect_center(layer=layers[0],
                                        offset=offset,
                                        height=via.mod.first_layer_height,
                                        width = 2 * self.m6_pitch - self.m6_width)
                    self.add_rect_center(layer=layers[-1],
                                        offset=offset,
                                        height=via.mod.second_layer_height,
                                        width = 2 * self.m6_pitch - self.m6_width)
    ...
    def add_via_stack_center(self,
        ...
        while cur_layer != to_layer:
            ...
            # min_area = min_area or ((cur_layer != from_layer) and (to_layer not in curr_stack))
            ...
            #     # offset_right[0] = offset[0] + self.m4_space/2
            #     # offset_right[1] = offset[1]
            #     offset_btm1 = vector(offset[0], offset[1] - 0.5 * self.m4_pitch).snap_to_grid()
            ...
            #     # offset_tl = [0,0]
            #     # offset_tl = vector(offset[0] - self.m4_space - via.mod.first_layer_width, offset[1]).snap_to_grid()
            #     offset_top1 = vector(offset[0], offset[1] + 0.5 * self.m4_pitch).snap_to_grid()
            #     # offset_tl[1] = offset[1]
            #     # offset_tl = offset_tl.snap_to_grid()
            #     print("offset, offset_topleft = ", offset, offset_top1)
            ...
            #     #                                 via.mod.first_layer_width,
            #     #                                 via.mod.first_layer_height)
            #     self.add_rect_center(layer="m4",
            #                         offset=offset,
            #                         width=via.mod.first_layer_width,
            #                         height = 4 * self.m4_pitch - self.m4_width)
            #     self.add_rect_center(layer="m5",
            #                         offset=offset,
            #                         width=via.mod.first_layer_width,
            #                         height = 4 * self.m4_pitch - self.m4_width)
            ...
            # else:
            print("add poly to m1 via stack offset = ", offset)
            via = self.add_via_center(layers=curr_stack,
                                    size=size,
                                    offset=offset,
                                    directions=directions,
                                    ...
                                    well_type=well_type,
                                    multiple_via4=multiple_via4,
                                    multiple_via5=multiple_via5)

            # Only add the enclosure if we are in an intermediate layer
            # or we are forced to
            if min_area or (cur_layer in intermediate_layers and cur_layer != "m4"):
                self.add_min_area_rect_center(cur_layer,
                                            offset,
                                            via.mod.first_layer_width,
                                            via.mod.first_layer_height)
            ...
            cur_layer = curr_stack[next_id]
        if to_layer == "m1" and min_area:
            self.add_min_area_rect_center(to_layer,
                                        offset,
                                        via.mod.first_layer_width,
                                        via.mod.first_layer_height)
    ...
    def add_min_area_rect_center(self,
        ...
        if preferred_directions[layer] == "V":
            ...
            # if layer == "m1":
            #     new_width = ceil(max(min_area / min_width, min_width))
            #     new_height = min_width
            # else:
            new_width = ceil(max(min_area / height, min_width))
            new_height = height
        print("min_area, new_height, new_width = ", min_area, new_height, new_width)
        debug.check(min_area <= round_to_grid_area(new_height*new_width), "Min area violated.")
    ...
    def gds_write_file(self, gds_layout):
        ...
        print("3.1. gds layout =", gds_layout)
        ...
        # print("1. gds write file.layoutObject.structures = ", writer.layoutObject.structures)
        # for structureName in writer.layoutObject.structures:
        #     print("structure name =", structureName)
        for i in self.objs:
            print("self.objs = ", i)
            i.gds_write_file(gds_layout)
            
        # print("2. gds write file.layoutObject.structures = ", writer.layoutObject.structures)
        # for structureName in writer.layoutObject.structures:
        #     print("structure name =", structureName)
        for pin_name in self.pin_map.keys():
        ...
        print("3.2. gds layout =", gds_layout)
        print("3.3. gds write file.layoutObject.structures = ", gdsMill.Gds2writer(gds_layout).layoutObject.structures)
        for structureName in gdsMill.Gds2writer(gds_layout).layoutObject.structures:
            print("structure name =", structureName)
        ...
        # print("4. gds write file.layoutObject.structures = ", writer.layoutObject.structures)
        # for structureName in writer.layoutObject.structures:
        #     print("structure name =", structureName)
        print("3.4. gds write file.layoutObject.structures = ", gdsMill.Gds2writer(gds_layout).layoutObject.structures)
        for structureName in gdsMill.Gds2writer(gds_layout).layoutObject.structures:
            print("structure name =", structureName)
        self.visited.append(self.name)
        print("3.5. gds write file.layoutObject.structures = ", gdsMill.Gds2writer(gds_layout).layoutObject.structures)
        for structureName in gdsMill.Gds2writer(gds_layout).layoutObject.structures:
            print("structure name =", structureName)
        # print("5. gds write file.layoutObject.structures = ", writer.layoutObject.structures)
        # for structureName in writer.layoutObject.structures:
        #     print("structure name =", structureName)
    ...
    def gds_write(self, gds_name):
        ...
        if not self.is_library_cell and self.visited:
            debug.info(3, "Creating layout structure {}".format(self.name))
            print("to call Vlsilayout")
            print("self.name =", self.name)
            self.gds = gdsMill.VlsiLayout(name=self.name, units=GDS["unit"])
            print("finished call Vlsilayout")
            print("self.gds =", self.gds)
            print("self.gds.structures =", self.gds.structures)
        print("to call Gds2writer")
        print("self.inst =", self.inst_names)
        writer = gdsMill.Gds2writer(self.gds)
        print("finished call Gds2writer")
        print("1. writer.layoutObject.structures = ", writer.layoutObject.structures)
        for structureName in writer.layoutObject.structures:
            print("structure name =", structureName)
        ...
        print("finished call clear_visited")
        print("2. writer.layoutObject.structures = ", writer.layoutObject.structures)
        for structureName in writer.layoutObject.structures:
            print("structure name =", structureName)
        # recursively create all the remaining objects
        self.gds_write_file(self.gds)

        print("4. writer.layoutObject.structures = ", writer.layoutObject.structures)
        for structureName in writer.layoutObject.structures:
            print("structure name =", structureName)
    ...
    def create_vertical_channel_route(self, netlist, offset, layer_stack, directions=None, minarea=False):
        """
        Wrapper to create a vertical channel route
        """
        from .channel_route import channel_route
        cr = channel_route(netlist, offset, layer_stack, directions, vertical=True, parent=self, minarea=False)
    ...
    def create_horizontal_channel_route(self, netlist, offset, layer_stack, directions=None, minarea=False):
        """
        Wrapper to create a horizontal channel route
        """
        from .channel_route import channel_route
        cr = channel_route(netlist, offset, layer_stack, directions, vertical=False, parent=self, minarea=False)
        # This causes problem in magic since it sometimes cannot extract connectivity of isntances
        # with no active devices.
        # self.add_inst(cr.name, cr)
        # self.connect_inst([])
    ...
    def get_bbox(self, side="all", margin=0):
        ...
        print("get bbox layout structures=", layout.structures)
    ...
    def copy_power_pins(self, inst, name, add_vias=True, new_name="", minarea=False, snap_to_grid=True):
        ...
        for pin in pins:
            if new_name == "":
                new_name = pin.name
            if pin.layer[-1] == "p":
                pin_layer = pin.layer[:-1]
            else:
                pin_layer = pin.layer
            if pin_layer in self.pwr_grid_layers:
                self.add_layout_pin(new_name,
                                    pin_layer,
                                    pin.ll(),
                                    pin.width(),
    ...
    def copy_power_pin(self, pin, loc=None, directions=None, new_name="",minarea=False, snap_to_grid=True):
        ...
        # Hack for min area
        if OPTS.tech_name == "sky130" or "tsmcN40":
            min_area = drc["minarea_{}".format(self.pwr_grid_layers[1])]
            width = round_to_grid(ceil(sqrt(min_area)))
            height = round_to_grid(ceil(min_area / width))
        else:
