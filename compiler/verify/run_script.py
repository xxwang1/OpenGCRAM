...
def run_script(cell_name, script="lvs"):
    ...
    if script == "lvs": 
        print("1. resultsfile = ", resultsfile)
    ...
    if script == "lvs": 
        print("2. resultsfile = ", resultsfile)
        # f = open(resultsfile, "r")
        # f.close()
    ...
    if script == "lvs": 
        print("3. resultsfile = ", resultsfile)
        # f = open(resultsfile, "r")
        # f.close()
    lastoutput = start
    ...
    while p.poll() == None:
        ...
        time.sleep(1)
        if script == "lvs": 
            print("4. resultsfile = ", resultsfile)
            # f = open(resultsfile, "r")
            # f.close()
    ...
    os.chdir(cwd)
    if script == "lvs": 
        print("5. resultsfile = ", resultsfile)
        # f = open(resultsfile, "r")
        # f.close()
