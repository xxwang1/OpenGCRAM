...
class local_gain_cell_array(gain_cell_base_array):
    """
    A local gain_cell array is a gain_cell array with a wordline driver.
    ...
    def __init__(self, rows, cols, rbl=None, left_rbl=None, right_rbl=None, name=""):
        ...
        debug.check(len(self.all_ports) < 3, "Local gain_cell array only supports dual port or less.")
        ...
        # the replica gain_cell in the control logic
    ...
    def add_modules(self):
        ...
        self.rwl_array = factory.create(module_type="gain_cell_wordline_buffer_array",
                                       rows=rows,
                                       cols=self.cols)
        self.wwl_array = factory.create(module_type="gain_cell_wordline_buffer_array",
                                       rows=rows,
                                       cols=self.cols)
    ...
    def add_pins(self):
        ...
        # Inputs to the gain_cell array (by port)
        ...
        self.all_bitline_names = self.gain_cell_array.all_bitline_names

        self.rbl_wordline_names = self.gain_cell_array.rbl_wordline_names
        self.all_rbl_wordline_names = self.gain_cell_array.all_rbl_wordline_names

        self.rbl_bitline_names = self.gain_cell_array.rbl_bitline_names
        self.all_rbl_bitline_names = self.gain_cell_array.all_rbl_bitline_names

        self.all_array_wordline_inputs = [x + "i" for x in self.gain_cell_array.get_all_wordline_names()]
    ...
    def create_instances(self):
        ...
        self.driver_wordline_outputs = []
        for port in self.all_ports:
            if port in self.read_ports: self.wl_insts.append(self.add_inst(name="rwl_driver{}".format(port),
                                               mod=self.rwl_array))
            elif port in self.write_ports: self.wl_insts.append(self.add_inst(name="wwl_driver{}".format(port),
                                               mod=self.wwl_array))
            temp = []
            if self.rbl[port] != 0:
        ...
        self.gain_cell_array_inst = self.add_inst(name="array",
    ...
    def place(self):
        """ Place the gain_celll array to the right of the wl driver. """

        # FIXME: Replace this with a tech specific parameter
        driver_to_array_spacing = 3 * self.m3_pitch

        wl_offset = vector(0, self.gain_cell_array.get_replica_bottom())
        self.wl_insts[0].place(wl_offset)

        gain_cell_array_offset = vector(self.wl_insts[0].rx() + driver_to_array_spacing, 0)
        self.gain_cell_array_inst.place(gain_cell_array_offset)

        if len(self.all_ports) > 1:
            rbl_wl_adder = self.cell.height * (self.rbl[1] != 0)
            wl_offset = vector(self.gain_cell_array_inst.rx() + self.wl_array.width + driver_to_array_spacing,
                               self.gain_cell_array.get_replica_bottom() + self.wl_array.height + rbl_wl_adder)
            self.wl_insts[1].place(wl_offset,
                                   mirror="XY")

        self.height = self.gain_cell_array.height
        self.width = max(self.gain_cell_array_inst.rx(), max([x.rx() for x in self.wl_insts]))

    def add_layout_pins(self):

        for x in self.get_inouts():
            self.copy_layout_pin(self.gain_cell_array_inst, x)

        supply_insts = [*self.wl_insts, self.gain_cell_array_inst]
    ...
    def route(self):
        ...
        global_wl_layer = layer_props.global_gain_cell_array.wordline_layer
        global_wl_pitch = getattr(self, "{}_pitch".format(global_wl_layer))
        global_wl_pitch_factor = layer_props.global_gain_cell_array.wordline_pitch_factor
        local_wl_layer = layer_props.local_gain_cell_array.wordline_layer
        ...
        for port in self.all_ports:
            ...
            for (driver_name, net_name) in zip(self.wl_insts[port].mod.get_outputs(), driver_outputs):
                ...
                in_pin = self.gain_cell_array_inst.get_pin(array_name)
                if port == 0:
                    out_loc = out_pin.rc()
    ...
    def get_main_array_top(self):
        return self.gain_cell_array_inst.by() + self.gain_cell_array.get_main_array_top()

    def get_main_array_bottom(self):
        return self.gain_cell_array_inst.by() + self.gain_cell_array.get_main_array_bottom()

    def get_main_array_left(self):
        return self.gain_cell_array_inst.lx() + self.gain_cell_array.get_main_array_left()

    def get_main_array_right(self):
        return self.gain_cell_array_inst.lx() + self.gain_cell_array.get_main_array_right()
    ...
    def get_column_offsets(self):
        ...
        offsets = [self.gain_cell_array_inst.lx() + x for x in self.gain_cell_array.get_column_offsets()]
        return offsets
    ...
    def graph_exclude_replica_col_bits(self):
        ...
        self.gain_cell_array.graph_exclude_replica_col_bits()
