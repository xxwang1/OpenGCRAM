...
class gain_cell_dff_buf(design):
    """
    ...
    def __init__(self, inv1_size=2, inv2_size=4, name=""):
        if name=="":
            name = "gain_cell_dff_buf_{0}".format(gain_cell_dff_buf.unique_id)
            gain_cell_dff_buf.unique_id += 1
        super().__init__(name)
    ...
    def create_layout(self):
        ...
        self.height = self.gain_cell_dff.height
        self.route_wires()
    ...
    def add_modules(self):
        ...
        self.inv1 = factory.create(module_type="pinv",
                                   size=self.inv1_size,
                                   height=self.gain_cell_dff.height)
    ...
    def create_instances(self):
        self.gain_cell_dff_inst=self.add_inst(name="gain_cell_dff_buf_gain_cell_dff",
                                    mod=self.gain_cell_dff)

        self.connect_inst(["D", "qint", "clk", "vdd", "gnd"])

        self.inv1_inst=self.add_inst(name="gain_cell_dff_buf_inv1",
                                     mod=self.inv1)
        self.connect_inst(["qint", "Qb", "vdd", "gnd"])

        self.inv2_inst=self.add_inst(name="gain_cell_dff_buf_inv2",
                                     mod=self.inv2)
        self.connect_inst(["Qb", "Q", "vdd", "gnd"])

    def place_instances(self):
        # Add the DFF
        self.gain_cell_dff_inst.place(vector(0, 0))
        ...
        self.inv1_inst.place(vector(self.gain_cell_dff_inst.rx() + well_spacing, 0))

        # Add INV2 to the right
        self.inv2_inst.place(vector(self.inv1_inst.rx()+self.nwell_enclose_implant, 0))
    ...
    def route_wires(self):
        ...
        # Route gain_cell_dff q to inv1 a
        q_pin = self.gain_cell_dff_inst.get_pin("Q")
    ...
    def add_layout_pins(self):
        ...
        vdd_pin=self.gain_cell_dff_inst.get_pin("vdd")
        ...
        gnd_pin=self.gain_cell_dff_inst.get_pin("gnd")
        ...
        clk_pin = self.gain_cell_dff_inst.get_pin("clk")
        ...
        din_pin = self.gain_cell_dff_inst.get_pin("D")
        ...
        self.add_via_stack_center(from_layer=dout_pin.layer,
                                  ...
                                  min_area=False)
        to_layer = "m2"
        if self.get_preferred_direction(to_layer) == "V":
            width = drc["minwidth_{}".format(to_layer)]
            height = ceil(max(drc("minarea_{}".format(to_layer)) / width, width))
        if self.get_preferred_direction(to_layer) == "H":
            height = drc["minwidth_{}".format(to_layer)]
            width = ceil(max(drc("minarea_{}".format(to_layer)) / height, height))
        correct_offset = vector(width, height).snap_to_grid()
        self.add_rect(layer=to_layer,
                                  offset=q_pos - correct_offset.scale(0.5,0.5),
                                  width=correct_offset.x,
                                  height=correct_offset.y)
        ...
        self.add_via_stack_center(from_layer=a2_pin.layer,
                                  ...
                                  min_area=False)
        a2_pin_to_layer = "m2"
        if self.get_preferred_direction(a2_pin_to_layer) == "V":
            width = drc["minwidth_{}".format(a2_pin_to_layer)]
            height = ceil(max(drc("minarea_{}".format(a2_pin_to_layer)) / width, width))
        if self.get_preferred_direction(a2_pin_to_layer) == "H":
            height = drc["minwidth_{}".format(a2_pin_to_layer)]
            width = ceil(max(drc("minarea_{}".format(a2_pin_to_layer)) / height, height))
        correct_offset = vector(width, height).snap_to_grid()
        self.add_rect(layer=a2_pin_to_layer,
                                  offset=qb_pos - correct_offset.scale(0.5,0.5),
                                  width=correct_offset.x,
                                  height=correct_offset.y)
