    ...
    def place_input_inverters(self):
        ...
        for inv_num in range(self.number_of_inputs):
            ...
            if (inv_num % 2 == 0):
                ...
                # y_off = inv_num * max(self.inv.find_highest_layer_coords("nwell").y, self.inv.height)
                mirror = "R0"
            else:
                y_off = (inv_num + 1) * self.inv.height
                # y_off = (inv_num + 1) * max(self.inv.find_highest_layer_coords("nwell").y, self.inv.height)
                mirror="MX"
    ...
    def place_and_array(self):
        ...
        for and_input in range(self.number_of_outputs):
            ...
            if (and_input % 2 == 0):
                ...
                # y_off = and_input * max(self.inv.find_highest_layer_coords("nwell").y, self.and_mod.height)
                mirror = "R0"
            else:
                y_off = (and_input + 1) * self.and_mod.height
                # y_off = (and_input + 1) * max(self.inv.find_highest_layer_coords("nwell").y, self.and_mod.height)
                mirror = "MX"
            ...
            for n in ["vdd", "gnd"]:
                ...
                for i in [self.inv_inst, self.and_inst]:
                    ...
                    for (bot_pin, top_pin) in zip(bot_pins, top_pins):
                        minarea = drc["minarea_{}".format(bot_pin.layer)]
                        if preferred_directions[bot_pin.layer] == "V":
                            width = bot_pin.width()
                            height = ceil(max(top_pin.uy() - self.bus_pitch, minarea / bot_pin.width()))
                        else:
                            height = top_pin.uy() - self.bus_pitch
                            width = ceil(max(minarea / (top_pin.uy() - self.bus_pitch), bot_pin.width()))
                ...
                for i in [self.inv_inst[0], self.inv_inst[-2], self.and_inst[0], self.and_inst[-2]]:
                    pins = i.get_pins(n)
                    for pin in pins:
                        self.copy_power_pin(pin, loc=pin.uc(),minarea=True)
            ...
            for num in range(0, self.number_of_outputs):
                for n in ["vdd", "gnd"]:
                    ...
                    for and_pin in and_pins:
                        ...
                        print("hierarchical_predecode add_power_pin pin, pin_pos, pin_layer = ", n, pin_pos, and_pin.layer)
                        self.add_power_pin(n, pin_pos, start_layer=and_pin.layer, minarea=True)
