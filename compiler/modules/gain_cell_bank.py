...
class gain_cell_bank(design):
    ...
    def create_netlist(self):
        ...
        self.compute_sizes()
        self.add_modules()
        self.add_pins() # Must create the replica gain_cell array first
    ...
    def create_layout(self):
        ...
        self.bank_array_ur = self.gain_cell_array_inst.ur()
        self.bank_array_ul = self.gain_cell_array_inst.ul()
    ...
    def add_pins(self):
        ...
        for port in self.read_ports:
            if self.has_rbl:
                self.add_pin("rbl_rbl_{0}_{0}".format(port), "OUTPUT")
        for port in self.write_ports:
            if self.has_rbl:
                self.add_pin("rbl_wbl_{0}_{0}".format(port), "INPUT")
        for port in self.write_ports:
            for bit in range(self.word_size + self.num_spare_cols):
    ...
    def route_bitlines(self, port):
        ...
        self.route_port_data_to_gain_cell_array(port)

    def create_instances(self):
        """ Create the instances of the netlist. """

        self.create_gain_cell_array()
    ...
    def compute_instance_offsets(self):
        ...
        self.main_gain_cell_array_left = self.gain_cell_array.get_main_array_left()
        # Just past the dummy column
        self.main_gain_cell_array_right = self.gain_cell_array.get_main_array_right()
        # Just past the dummy row and replica row
        self.main_gain_cell_array_bottom = self.gain_cell_array.get_main_array_bottom()
    ...
    def get_column_offsets(self):
        ...
        """
        # Assumes gain_cell_array is at 0,0
        offsets = self.gain_cell_array.get_column_offsets()
        return offsets
    ...
    def compute_instance_port0_offsets(self):
        ...
        # gain_cell array is placed at (0,0)
        self.gain_cell_array_offset = vector(0, 0)

        # LOWER RIGHT QUADRANT
        # Below the gain_cell array
        self.port_data_offsets[port] = vector(0, 0)

        # UPPER LEFT QUADRANT
        # To the left of the gain_cell array above the predecoders and control logic
        x_offset = self.decoder_gap + self.port_address[port].width + self.m6_space
        self.port_address_offsets[port] = vector(-x_offset,
                                                 self.main_gain_cell_array_bottom)
    ...
    def compute_instance_port1_offsets(self):
        ...
        # UPPER LEFT QUADRANT
        # Above the gain_cell array
        self.port_data_offsets[port] = vector(0, self.gain_cell_array_top)

        # LOWER RIGHT QUADRANT
        # To the right of the gain_cell array
        x_offset = self.gain_cell_array_right + self.port_address[port].width + self.decoder_gap + self.m6_space
        self.port_address_offsets[port] = vector(x_offset,
                                                 self.main_gain_cell_array_bottom)
    ...
    def place_instances(self):
        ...
        self.place_gain_cell_array(self.gain_cell_array_offset)
    ...
    def add_modules(self):
        ...
        if local_array_size > 0:
            ...
            self.gain_cell_array = factory.create(module_type="global_gain_cell_array",
                                                cols=cols,
                                                rows=self.num_rows,
                                                rbl=rbl,
                                                left_rbl=left_wbl,
    ...
    def create_gain_cell_array(self):
        """ Creating gain_cell Array """
    ...
    def place_gain_cell_array(self, offset):
        """ Placing gain_cell Array """
        self.gain_cell_array_inst.place(offset)
    ...
    def create_port_data(self):
        ...
        for port in self.all_ports:
            self.port_data_inst[port]=self.add_inst(name="gain_cell_port_data{}".format(port),
    ...
    def route_supplies(self):
        ...
        # (this won't add vias to internal gain_cell pins, for example)

        # This avoids getting copy errors on vias and other instances
        all_insts = [self.gain_cell_array_inst] + self.port_address_inst + self.port_data_inst
        ...
        if 'vpb' in self.gain_cell_array_inst.mod.pins and 'vnb' in self.gain_cell_array_inst.mod.pins:
            for pin_name, supply_name in zip(['vnb', 'vpb'], ['gnd', 'vdd']):
                self.copy_layout_pin(self.gain_cell_array_inst, pin_name, new_name=supply_name)
    ...
    def route_central_bus(self):
        ...
        # The control bus is routed up to two pitches below the gain_cell array
        ...
        # Port 1
        if len(self.all_ports)==2:
            # The other control bus is routed up to two pitches above the gain_cell array
            control_bus_length = self.max_y_offset - self.port_data_inst[1].by()
            control_bus_offset = vector(self.gain_cell_array_right + 2.5 * self.m3_pitch,
    ...
    def route_port_data_to_gain_cell_array(self, port):
        ...
        for (array_name, data_name) in zip(inst1_bl_name, inst2_bl_name):
    ...
    def route_port_address(self, port):
        """ Connect Wordline driver to gain_cell array wordline """
    ...
    def route_port_address_out(self, port, side="left"):
        ...
        # if side == "left":
        wl_name = self.gain_cell_array.get_wordline_names(port) + [rbl_wl_name]
        # else:
        #     wl_name = self.gain_cell_array.get_wordline_names(gain_cell_port)[0] + [rbl_wl_name]
        for (driver_name, array_name) in zip(driver_names, wl_name):
    ...
    def add_lvs_correspondence_points(self):
        ...
        for pin_name in self.gain_cell_array.get_all_bitline_names():
            self.copy_layout_pin(self.gain_cell_array, pin_name)
        # Add the wordline names
        # for i in range(self.num_rows):
        #     wl_name = "wl_{}".format(i)
        #     wl_pin = self.gain_cell_array_inst.get_pin(wl_name)
        ...
        #     bl_pin = self.gain_cell_array_inst.get_pin(bl_name)
        #     br_pin = self.gain_cell_array_inst.get_pin(br_name)
    ...
    def get_cell_name(self, inst_name, row, col):
        ...
        """
        return self.gain_cell_array_inst.mod.get_cell_name(inst_name + "{}x".format(OPTS.hier_seperator) + self.gain_cell_array_inst.name,
                                                         row,
                                                         col)
    ...
    def clear_exclude_bits(self):
        ...
        """
        self.gain_cell_array.clear_exclude_bits()
