...
class delay_chain(design):
    ...
    def create_layout(self):
        ...
        self.width = (max(self.fanout_list) + 1) * self.inv.width + max(self.fanout_list) * self.nwell_enclose_implant
    ...
    def place_inverters(self):
        ...
        for stage_num, fanout_size in zip(range(self.rows), self.fanout_list):
            ...
            for i in range(fanout_size):
                inv_offset += vector(self.inv.width + self.nwell_enclose_implant, 0)
    ...
    def route_inverters(self):
        ...
        for i in range(len(self.driver_inst_list)):
            ...
            self.add_rect(layer=to_layer,
                                    offset=a_pin.center() - correct_offset.scale(0.5,0.5),
    ...
    def route_supply_rails(self):
        ...
        for stage_num, fanout_size in zip(range(self.rows), self.fanout_list):
            ...
            if OPTS.tech_name == "sky130":
                for name in ["vdd", "gnd"]:
                    for inst in [self.inv_inst]:
                        self.copy_layout_pin(inst, name)
            else:
                if inv_mirror == "R0":
                    self.add_layout_pin_rect_center(text="gnd",
                                                    layer=self.route_layer,
                                                    offset=vector(0.5 * self.width, self.inv.height * stage_num),
                                                    width=self.width)
                    self.add_layout_pin_rect_center(text="vdd",
                                                    layer=self.route_layer,
                                                    offset=vector(0.5 * self.width, self.inv.height * (stage_num + 1)),
                                                    width=self.width)
                else: 
                    self.add_layout_pin_rect_center(text="vdd",
                                                    layer=self.route_layer,
