...
from openram.base.utils import round_to_grid, ceil, sqrt
...
class pgate(design):
    ...
    def __init__(self, name, height=None, add_wells=True):
        ...
        self.top_bottom_space = max(contact_to_vdd_rail_space,
                                    poly_to_poly_gate_space, self.implant_enclose)
    ...
    def connect_pin_to_rail(self, inst, pin_name, supply_name):
        ...
        for source_pin in source_pins:
            ...
            if source_pin.width() > 0: width = round(source_pin.width(), 3)
            else: width = round(abs(source_pin.width()), 3) * (-1)
            self.add_rect(layer=source_pin.layer,
                          offset=source_pin.ll(),
                          height=height,
                          width=width)

    def route_input_gate(self, pmos_inst, nmos_inst, ypos, name, position="left", directions=None, minarea=True):
        """
        ...
        via = self.add_via_stack_center(from_layer="poly",
                                        ...
                                        min_area=minarea)
        print("via.mod.second_layer_width = ", round(via.mod.second_layer_width, 3))
        print("via.mod.second_layer_height = ", round(via.mod.second_layer_height, 3))
        print("self.poly_contact.first_layer_width = ", round(self.poly_contact.first_layer_width, 3))
        print("left_gate_offset.x - contact_offset.x = ", round(left_gate_offset.x - contact_offset.x, 3))
        print("contact_offset = ", contact_offset)
        if name == "B" and minarea==False:
            height = drc["minwidth_m1"]
            width = ceil(max(drc["minarea_m1"] / height, height))
        else: 
            width=round(via.mod.second_layer_width, 3)
            height=round(via.mod.second_layer_height, 3)
        ...
        self.add_rect_center(layer="poly",
                             offset=mid_point,
                             height=round(self.poly_contact.first_layer_width, 3),
                             width=round(left_gate_offset.x - contact_offset.x, 3))
    ...
    def extend_wells(self):
        ...
        if "nwell" in layer:
            ...
            nwell_max_offset = max(self.find_highest_layer_coords("nwell").y,
                                   full_height)
            self.nwell_position = vector(0, self.nwell_yoffset) - vector(self.well_extend_active, 0)
            self.nwell_height = nwell_max_offset - self.nwell_yoffset
            self.add_rect(layer="nwell",
                          offset=self.nwell_position,
                          width=self.width + 1 * self.well_extend_active,
        ...
        if "pwell" in layer:
            pwell_min_offset = min(self.find_lowest_layer_coords("pwell").y,
                                   -0.5 * self.m1_width)
            self.pwell_position = vector(-self.well_extend_active, pwell_min_offset)
            self.pwell_height = self.nwell_yoffset - self.pwell_position.y
        ...
        #add implant to enclose poly
        
        print("cell_props.pgate.add_implants =", cell_props.pgate.add_implants)
    ...
    def add_nwell_contact(self, pmos, pmos_pos):
        ...
        #                   + self.active_space
        contact_xoffset = pmos_pos.x + pmos.well_width \
                          + self.active_space
        ...
        contact_yoffset = min(self.height - 0.5 * self.implant_width,
                             ...
                             - self.implant_enclose_active * 0.9
        print("running add_nwell_contact")
        print("contact_yoffset = ", contact_yoffset)
        print("pmos.active_contact =", pmos.active_contact)
        print("self.height, self.implant_width, self.get_tx_insts(pmos)[0].uy(), pmos.active_contact.first_layer_height, self.implant_enclose_active = ", self.height, self.implant_width, self.get_tx_insts("pmos")[0].uy(), pmos.active_contact.first_layer_height, self.implant_enclose_active)
        ...
        self.nwell_contact = self.add_via_center(layers=layer_stack,
                                                 ...
                                                 well_type="n",
                                                 directions=("V", "V"))
        print("rect_center offset = ", contact_offset + vector(0, 0.5 * (self.height - contact_offset.y)))
        print("rect_center layer = ", self.route_layer)
        print("rect_center width, height = ", self.nwell_contact.mod.second_layer_width, self.height - contact_offset.y)
    ...
    def add_nimplant_enclose(self, nmos, nmos_pos):
        # offset = vector(self.nwell_position.x, self.top_bottom_space - self.implant_enclose)
        offset = vector(nmos.active_offset.x - nmos.enclose_width, self.top_bottom_space - self.implant_enclose)
        # offset = vector(self.nwell_position.x, min(self.find_lowest_layer_coords("pwell").y,
                                #    -0.5 * self.m1_width))
        height = self.pwell_height - (offset.y - self.pwell_position.y) 
        # width = nmos_pos.x + nmos.well_width
        width = nmos.nimplant_width + nmos_pos.x - nmos.active_offset.x
        self.add_rect(layer="nimplant",
                      offset=offset,
                          width=width,
                          height=height)
    ...
    def add_pimplant_enclose(self, pmos, pmos_pos):
        # offset = pmos.pimplant_offset
        # height = self.nwell_height - (offset.y - self.nwell_position.y) 
        # width = pmos.pimplant_width
        # self.add_rect(layer="pimplant",
        #             offset=offset,
        #                 width=width,
        #                 height=height)
        height = pmos.active_height + pmos.implant_enclose + pmos_pos.y - self.nwell_position.y
        # offset = vector(pmos_pos.x - pmos.enclose_width, self.nwell_position.y)
        offset = vector(pmos.active_offset.x - pmos.enclose_width, self.nwell_position.y)
        width = pmos.pimplant_width + pmos_pos.x - pmos.active_offset.x
        # self.add_rect(layer="pimplant",
        #             offset=offset,
        #                 width=pmos_pos.x + pmos.well_width,
        #                 height=height)
        self.add_rect(layer="pimplant",
                    offset=offset,
                        width=width,
    ...
    def extend_implants(self):
        ...
        self.add_rect(layer="nimplant",
                      ...
                      width=self.width,
                      height=self.implant_width)
        # print("self.implant_enclose_poly = ", self.implant_enclose_poly)
...
#            pass
    ...
    def add_pwell_contact(self, nmos, nmos_pos):
        ...
        # contact_xoffset = nmos_pos.x + nmos.active_width \
                        #   + self.active_space
        contact_xoffset = nmos_pos.x + nmos.well_width \
                          + self.active_space
        # Allow an nimplant below it under the rail
        print("running add_pwell contact")
        print("contact_yoffset 1,2,3 = ", 0.5 * self.implant_width + self.implant_enclose_active,
                              self.get_tx_insts("nmos")[0].by(), self.implant_enclose_active)
