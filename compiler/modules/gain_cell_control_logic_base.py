...
class control_logic_base(design):
    ...
    def place_gated_clk_bar_row(self, row):
        ...
        x_offset = self.place_util(self.gated_clk_bar_inst, x_offset+self.nwell_enclose_implant, row)
    ...
    def connect_output(self, inst, pin_name, out_name):
        ...
        out_pos = out_pin.center()
        # if pin_name == "Z": out_pos += vector(0.5 * drc("minwidth_{}".format(out_pin.layer)), 0)
    ...
    def route_supplies(self):
        ...
        # print("supplu_layer = ", supply_layer)
        # print("row end inst = ", self.row_end_inst)
    ...
    def place_util(self, inst, x_offset, row, pp_space=False):
        """ Utility to place a row and compute the next offset """

        (y_offset, mirror) = self.get_offset(row)
        # if pp_space: y_offset += self.implant_space
    ...
    def route_output_to_bus_jogged(self, inst, name):
        ...
        print("running route_output_to_bus_jogged")
        ...
        print("[out_pos, mid1, mid2, bus_pos] = ", [out_pos, mid1, mid2, bus_pos])
        print("out_pin.layer = ", out_pin.layer)
