...
class global_gain_cell_array(gain_cell_base_array):
    """
    Creates a global gain_cell array.
    ...
    def __init__(self, rows, cols, rbl=None, left_rbl=None, right_rbl=None, name=""):
        ...
        debug.check(len(self.all_ports) < 3, "Only support dual port or less in global gain_cell array.")
    ...
    def add_modules(self):
        ...
        if len(self.column_sizes) == 1:
            la = factory.create(module_type="local_gain_cell_array",
                                rows=self.row_size,
    ...
    def add_bitline_pins(self):
        # FIXME: aren't these already defined via inheritence by gain_cell base array?
        ...
        for col in range(self.column_size):
            for port in self.read_ports:
                self.bitline_names[port].append("rbl_{0}_{1}".format(port, col))
            for port in self.write_ports:
                self.bitline_names[port].append("wbl_{0}_{1}".format(port, col))
    ...
    def add_wordline_pins(self):
        ...
        for bit in self.all_ports:
            if self.rbl[bit] == 0:
                continue
            for port in self.read_ports:
                self.rbl_wordline_names[port].append("rbl_rwl_{0}_{1}".format(port, bit))
            for port in self.write_ports:
                self.rbl_wordline_names[port].append("rbl_wwl_{0}_{1}".format(port, bit))
        ...
        for row in range(self.row_size):
            for port in self.read_ports:
                self.wordline_names[port].append("rwl_{0}_{1}".format(port, row))
            for port in self.write_ports:
                self.wordline_names[port].append("wwl_{0}_{1}".format(port, row))
    ...
    def create_instances(self):
        ...
        for col, mod in zip(self.col_offsets, self.local_mods):
            ...
            port_inouts = [x for x in mod.get_inouts() if x.startswith("rbl") or x.startswith("wbl")]
    ...
    def add_layout_pins(self):
        ...
        for col, inst in zip(self.col_offsets, self.local_insts):
            for port in self.all_ports:
                port_inouts = [x for x in inst.mod.get_inouts() if (x.startswith("rbl_{}".format(port)) and port in self.read_ports) or (x.startswith("br_{}".format(port)) and port in self.write_ports)]
                for pin_name in port_inouts:
                    # Offset of the last underscore that defines the bit number
                    bit_index = pin_name.rindex('_')
                    # col is the bit offset of the local array,
        ...
        wl_layer = layer_props.global_gain_cell_array.wordline_layer
        ...
        if len(self.rbls) > 0:
            # Replica bitlines
            for port in self.read_ports:
                self.copy_layout_pin(self.local_insts[0], "rbl_rbl_0_0")
            # for port in self.write_ports:
            #     self.copy_layout_pin(self.local_insts[0], "rbl_wbl_0_0")

            if len(self.all_ports) > 1:
                self.copy_layout_pin(self.local_insts[0], "rbl_rbl_1_0")
                # self.copy_layout_pin(self.local_insts[0], "rbl_br_1_0")
                # self.copy_layout_pin(self.local_insts[-1], "rbl_bl_0_1")
                self.copy_layout_pin(self.local_insts[-1], "rbl_wbl_0_1")
                # self.copy_layout_pin(self.local_insts[-1], "rbl_bl_1_1")
                self.copy_layout_pin(self.local_insts[-1], "rbl_wbl_1_1")
    ...
    def get_cell_name(self, inst_name, row, col):
        ...
        for i, local_col in enumerate(self.col_offsets):
            ...
            # In this case, we it should be in the last gain_cell array
