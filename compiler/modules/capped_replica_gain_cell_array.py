...
class capped_replica_gain_cell_array(gain_cell_base_array):
    """
    Creates a replica gain_cell array then adds the row and column caps to all
    sides of a gain_cell array.
    """
    def __init__(self, rows, cols, rbl=None, left_rbl=None, right_rbl=None, name=""):
        ...
        # If we aren't using row/col caps, then we need to use the gain_cell
    ...
    def add_modules(self):
        ...
        self.replica_gain_cell_array = factory.create(module_type="replica_gain_cell_array",
        ...
        # Dummy Row or Col Cap, depending on gain_cell array properties
        col_cap_module_type = ("col_cap_array" if self.cell.end_caps else "dummy_gain_cell_array")
        ...
        # Dummy Col or Row Cap, depending on gain_cell array properties
        row_cap_module_type = ("row_cap_array" if self.cell.end_caps else "dummy_gain_cell_array")
    ...
    def add_bitline_pins(self):
        ...
        self.bitline_names = self.replica_gain_cell_array.bitline_names
        self.all_bitline_names = self.replica_gain_cell_array.all_bitline_names
        self.rbl_bitline_names = self.replica_gain_cell_array.rbl_bitline_names
        self.all_rbl_bitline_names = self.replica_gain_cell_array.all_rbl_bitline_names

        # this one is actually used (obviously)
        self.bitline_pin_list = self.replica_gain_cell_array.bitline_pin_list
    ...
    def add_wordline_pins(self):
        ...
        self.rbl_wordline_names = self.replica_gain_cell_array.rbl_wordline_names
        self.all_rbl_wordline_names = self.replica_gain_cell_array.all_rbl_wordline_names
        self.wordline_names = self.replica_gain_cell_array.wordline_names
        self.all_wordline_names = self.replica_gain_cell_array.all_wordline_names

        self.used_wordline_names = self.replica_gain_cell_array.used_wordline_names
        self.unused_wordline_names = self.replica_gain_cell_array.unused_wordline_names
        self.replica_array_wordline_names_with_grounded_wls = ["gnd" if x in self.unused_wordline_names else x for x in self.replica_gain_cell_array.wordline_pin_list]
    ...
    def create_instances(self):
        ...
        for i in range(0, 2, 2):
            bl.append(self.bitline_pin_list[i+1])
            bl.append(self.bitline_pin_list[i])
        bl.extend(self.bitline_pin_list[2:-2])
        for i in range(len(self.bitline_pin_list) - 2, len(self.bitline_pin_list), 2):
            bl.append(self.bitline_pin_list[i+1])
            bl.append(self.bitline_pin_list[i])
        print("capped_replica_gain_cell_array self.bitline_pin_list = ", self.bitline_pin_list)
        self.replica_gain_cell_array_inst=self.add_inst(name="replica_gain_cell_array",
                                                      mod=self.replica_gain_cell_array)
        ...
        for i in range(4, len(self.wordline_pin_list) - 4, 2):
    ...
    def create_layout(self):
        ...
        print("self.supply_stack = ", self.supply_stack)
        self.horizontal_pitch = 1.1 * getattr(self, "{}_pitch".format(self.supply_stack[0]))
        self.vertical_pitch = 1.1 * getattr(self, "{}_pitch".format(self.supply_stack[2]))
        print("self.vertical_pitch = ", self.vertical_pitch)
        print("self.horizontal_pitch = ", self.horizontal_pitch)
        ...
        self.replica_gain_cell_array_inst.place(offset=self.unused_offset)

        self.add_end_caps()

        # shift everything up and right to account for cap cells
        self.translate_all(self.gain_cell_offset.scale(-1, -1))
    ...
    def get_main_array_top(self):
        return self.replica_gain_cell_array_inst.by() + self.replica_gain_cell_array.get_main_array_top()

    def get_main_array_bottom(self):
        return self.replica_gain_cell_array_inst.by() + self.replica_gain_cell_array.get_main_array_bottom()

    def get_main_array_left(self):
        return self.replica_gain_cell_array_inst.lx() + self.replica_gain_cell_array.get_main_array_left()

    def get_main_array_right(self):
        return self.replica_gain_cell_array_inst.lx() + self.replica_gain_cell_array.get_main_array_right()
    ...
    def get_column_offsets(self):
        ...
        offsets = [self.replica_gain_cell_array_inst.lx() + x for x in self.replica_gain_cell_array.get_column_offsets()]
        return offsets
    ...
    def add_end_caps(self):
        ...
        dummy_row_offset = self.gain_cell_offset.scale(0, flip_dummy) + self.replica_gain_cell_array_inst.ul()
        self.dummy_row_insts[1].place(offset=dummy_row_offset,
                                      mirror="MX" if flip_dummy else "R0")

        # Far bottom dummy row (first row below array IS flipped)
        flip_dummy = (self.rbl[0] + 1) % 2
        dummy_row_offset = self.gain_cell_offset.scale(0, flip_dummy - 1) + self.unused_offset
        self.dummy_row_insts[0].place(offset=dummy_row_offset,
                                      mirror="MX" if flip_dummy else "R0")
        # Far left dummy col
        # Shifted down by the number of left RBLs even if we aren't adding replica column to this gain_cell array
        dummy_col_offset = self.gain_cell_offset.scale(-1, -1) + self.unused_offset
        self.dummy_col_insts[0].place(offset=dummy_col_offset)

        # Far right dummy col
        # Shifted down by the number of left RBLs even if we aren't adding replica column to this gain_cell array
        dummy_col_offset = self.gain_cell_offset.scale(0, -1) + self.replica_gain_cell_array_inst.lr()
    ...
    def route_supplies(self):
        ...
        if OPTS.gain_cell == "pgain_cell":
            gain_cell = factory.create(module_type="pgain_cell")
        else:
            gain_cell = getattr(props, "gain_cell_{}port".format(OPTS.num_ports))

        vdd_dir = gain_cell.vdd_dir
        print("vdd_dir = ", vdd_dir)
        ...
        for inst in supply_insts:
            ...
            for pin in inst.get_pins("vdd"):
                if vdd_dir == "V":
                    ...
                    print("self.left_vdd_locs[0].x = ", self.left_vdd_locs[0].x)
                    print("self.right_vdd_locs[0].x = ", self.right_vdd_locs[0].x)
    ...
    def route_vertical_side_pin(self, name, side, offset_multiple=1):
        ...
        print("route_vertical_side_pin")
        print("bot_loc = ", bot_loc)
        print("top_loc = ", top_loc)
        layer = self.supply_stack[2]
        top_via = contact(layer_stack=self.supply_stack,
                          directions=("H", "H"))
        print("route_vertical_side_pin layer, supply_stack = ", layer, self.supply_stack)
        print("bot_loc, top_loc = ", bot_loc, top_loc)
    ...
    def route_horizontal_side_pin(self, name, side, offset_multiple=1):
        ...
        print("Route_horizontal_side_pin")
    ...
    def connect_horizontal_side_pin(self, pin, yoffset):
        ...
        print("Running connect_horizontal_side_pin")
        print("pin_loc, pin.layer=", pin_loc, pin.layer)
    ...
    def connect_vertical_side_pin(self, pin, xoffset):
        ...
        print("cell_loc, pin_loc = ", cell_loc, pin_loc)
        print("from_layer, to_layer = ", pin.layer, self.supply_stack[2])
        # Place the pins a track outside of the array
        print("Running connect_vertical_side_pin")
        print("pin_loc, pin.layer=", pin_loc, pin.layer)
        self.add_via_stack_center(offset=pin_loc,
                                  from_layer=pin.layer,
    ...
    def graph_exclude_bits(self, targ_row=None, targ_col=None):
        ...
        """
        self.replica_gain_cell_array.graph_exclude_bits(targ_row, targ_col)

    def graph_exclude_replica_col_bits(self):
        """
        Exclude all replica/dummy cells in the replica columns except the replica bit.
        """
        self.replica_gain_cell_array.graph_exclude_replica_col_bits()

    def get_cell_name(self, inst_name, row, col):
        """
        Gets the spice name of the target gain_cell.
        """
        return self.replica_gain_cell_array.get_cell_name(inst_name + "{}x".format(OPTS.hier_seperator) + self.replica_gain_cell_array_inst.name, row, col)

    def clear_exclude_bits(self):
        """
        Clears the bit exclusions
        """
        self.replica_gain_cell_array.clear_exclude_bits()
