...
class gain_cell_predischarge_array(design):
    """
    Dynamically generated predischarge array of all bitlines.  Cols is number
    ...
    def add_modules(self):
        self.pc_cell = factory.create(module_type=OPTS.gain_cell_predischarge,
                                      size=self.size,
    ...
    def add_layout_pins(self):
        ...
        self.route_horizontal_pins("en", layer=self.en_layer)
        for inst in self.local_insts:
            self.add_via_stack_center(from_layer=en_pin.layer,
                                      to_layer=self.en_layer,
                                      offset=inst.get_pin("en").center())
    ...
    def route_supplies(self):
        print("Gain Cell predischarge array routing supplies")
        self.route_horizontal_pins("gnd", snap_to_grid=True)
        print("Gain Cell predischarge array routing supplies end")
    def create_insts(self):
        """Creates a predischarge array by horizontally tiling the predischarge cell"""
        self.local_insts = []
        for i in range(self.columns):
            ...
            self.connect_inst([self.gain_cell_rbl+"_{0}".format(i), "en", "gnd"])

    def place_insts(self):
        """ Places predischarge array by horizontally tiling the predischarge cell"""
